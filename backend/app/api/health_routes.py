...
async def health_check(db: AsyncSession = Depends(get_db), user_id=Depends(auth_dependency)):
...
async def get_stats(db: AsyncSession = Depends(get_db), user_id=Depends(auth_dependency)):
